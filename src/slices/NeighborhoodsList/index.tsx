...
const NeighborhoodsList: FC<NeighborhoodsListProps> = ({ slice }) => {
  // Forçage typé sur le champ groupé "name"
  const neighborhoods = slice.primary.name as { title?: { text: string }[] }[]
            ...
            {neighborhoods.map((item, index) => {
              ...
              return (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className="relative rounded-2xl p-6 shadow-xl group hover:shadow-2xl transition-all duration-300 bg-white border border-gray-100"
                >
                  ...
                  <p className="text-sm text-gray-500 mt-2">Quartier attractif du 15ᵉ</p>
                </motion.div>
              )
            })}
          </div>
